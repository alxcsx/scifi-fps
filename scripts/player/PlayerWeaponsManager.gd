...
func _ready() -> void:        
  ...
  for w in weapons:
    ...
    if data and inventory.unlocked_weapons.has(data.weapon_id):
      w.is_unlocked = true
...
func _shoot() -> void:
  ...
  if inventory.ammo_inventory.has(data.ammo_type) and inventory.ammo_inventory[data.ammo_type] >= data.ammo_cost:
    ...
    print("Fired weapon: %s, dealing %f damage!" % [data.weapon_id, data.damage])
...
func _on_weapon_unlocked(weapon_id: String) -> void:
  var id := weapons.find_custom(func(w: BaseWeaponView): return w.weapon_data.weapon_id == weapon_id)
  print("Weapon unlocked: %s (%d)" % [weapon_id, id])
  if id != -1:
    weapons[id].is_unlocked = true
    equip_weapon(id)
  else:
    push_warning("Unlocked weapon '%s' not found in weapons list!" % weapon_id)
