...
func setup_ammo_display() -> void:
  if not ammo_label:
    ...
    weapons_manager.ammo_changed.connect(_on_ammo_changed)
    _update_ammo_display()
  else:
    ammo_label.hide()

func _on_ammo_changed(ammo_type: AmmoItem.AmmoType, _magazine: int, _reserve: int) -> void:
...
func _update_ammo_display() -> void:
  var ammo_status := weapons_manager.get_current_ammo_status()

  if not ammo_status or ammo_status["type"] == AmmoItem.AmmoType.NONE:
    ammo_label.text = "---"
    return

  ammo_label.text = "%d / %d" % [ammo_status["magazine"], ammo_status["reserve"]]
