...
func process_movement(player: CharacterBody3D, delta: float) -> Vector3:
	...
	if direction != Vector3.ZERO:
		...
		hvel.x = move_toward(hvel.x, target_velocity.x, current_accel * delta)
		hvel.z = move_toward(hvel.z, target_velocity.z, current_accel * delta)
	else:
		# Aggressive friction when letting go of the keys
		hvel.x = move_toward(hvel.x, 0.0, current_friction * delta)
		hvel.z = move_toward(hvel.z, 0.0, current_friction * delta)
