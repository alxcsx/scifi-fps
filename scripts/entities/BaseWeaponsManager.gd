...
func _execute_combat(origin: Vector3, direction: Vector3) -> void:
  ...
  print("Attacker (%s) position: %s. Direction: %s" % [attacker.name, origin, direction])
  ...
  while hits < max_hits:
    ...
    if result:
      ...
      if target is BaseEntity and target != attacker:
        target.take_damage(hit_payload.clone({"damage": current_damage}))
        current_damage = current_damage * hit_payload.pierce_damage_retention
